    ...
    $.extend(ProduceData.prototype, {
        ...
        _createRightNodes: function(obj) {
            ...
            for(i = 0; i < len; i++) {
                ...
                ?
                    '<li class="prices" data-id="' + obj[i].id + '" data-price="' + obj[i].point_money + '"><strong>' + obj[i][numKey] + '</strong>' + '元/' + obj[i].point_name.replace(/会员套餐|享受型/, '') + '</li>'
                : 
        ...
        _createLeftNodes: function(obj) {
            ...
            for(i = 0; i < len; i++) {
                ...
                var nodes = obj[i].limited
                    ...
                    ?
                        '<li class="boxes" data-price="' + obj[i]["point_money"] + '"><header class="vip-header" data-type="'  + obj[i].type + '"><h2 class="vip-type">'  + obj[i].point_name.replace('套餐', '') + '</h2></header><main class="details"><p class="price"><strong>' + obj[i][numKey] + '</strong>' + '元/' + obj[i].point_name.replace(/会员套餐|享受型/, '') + '</p><ul><li class="times">次数限制：'+ obj[i].download_limit +'个/天</li>{{ lis-items }}</ul></main><footer><input type="button" value="立即订购" data-price="'  + obj[i]['point_money'] + obj[i].unit + '"></footer></li>'

                    :
                        '<li class="boxes" data-price="' + obj[i]["point_money"] + '"><header class="vip-header" data-type="'  + obj[i].type + '"><h2 class="vip-type">'  + obj[i].gap + '</h2></header><main class="details"><p class="price"><strong>' + obj[i][numKey] + '</strong>' + obj[i].unit + '</p><ul>{{ lis-items }}</ul></main><footer><input type="button" value="立即订购" data-price="'  + obj[i]['point_money'] + obj[i].unit + '"></footer></li>';

                $(hooks[0]).append(nodes.replace('{{ lis-items }}', function(){
                			var html = '';
                        	$.each(obj[i]["remark"],function(index, el) {
                        		html += '<li>'+ el +'</li>';
                        	});
                        	return html;
                        }));
        ...
        _callPay: function() {
            var $calledNode = $('#box-right-lists').find('.current'),
                ...
                couponsH = '<li><span class="discount-num">￥{ price }</span><div class="coupon-info"><p>满{ min_price }可用</p><p>{ start_time }-{ end_time }</p><span class="useCoupon" data-id="{ id }" data-price="{ p_price }">使用</span></div></li>',
                html = '';
