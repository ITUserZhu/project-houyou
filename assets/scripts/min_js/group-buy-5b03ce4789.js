...
$(function() {
	...
	$ipt.on('keyup', function() {
		var num = Number($(this).val()),
			leftNum = $(this).attr('data-leftNum');
		if(num <= 0 ) {
			$(this).val(1)
		}else if (num > leftNum) {
			$(this).val(leftNum)
	...
	$join_entry.on('click', function(e) {
		var $target = $(e.target).closest('a'),
			_this = $(this),
			_ipt = _this.find('.ipt-num'),
			_leftNum = _ipt.attr('data-leftNum'),
			curNum;
		curNum = Number(_ipt.val());
		...
		if($target.is('.other-num')) {
			_ipt.val($target.text()/1>_leftNum?_leftNum:$target.text());
			return;
		}
	});
    ...
    $joinBtn.click(function(e) {
    	var ipt_val = $(e.target).closest('.join-entry').find('.ipt-num').val(),
    		type = $(e.target).closest('.join-entry').attr('data-type');
    	var data = {
    		people_num: ipt_val,
    		vip_type: type
    	};
    ...
    function getCode(data) {
    	$.ajax({
    		...
    		data: data,
    		beforeSend: function() {
    			$payWra.find('.tips em').text(data.people_num);
    			$payWra.find('.pay-price').text(data.people_num);
    ...
    var $topCheckLi = $('.vip-type-check').children('li');
    $topCheckLi.on('mouseover', function(e) {
    	var $target = $(e.target),
    		_index = $target.index();
		__commonToggleActive([$(this), $('.join-center').eq(_index)])
    });
