...
$(function() {
    ...
    var recordsTemplate = '<tr><td>{{ time }}</td><td>{{ type }}</td><td>{{ money }}元</td><td>{{ content }}</td></tr>';
    ...
    var couponHtml = '<li class="{{ is_cover }}"><div class="dis-con"><span class="dis-num"><i>￥</i>{{ price }}</span><p>满{{ min_price }}可用</p><p class="time">{{ start_time }}-{{ end_time }}</p></div><div class="dis-bot"><span class="use"><a href="/Mac/vip">立即使用</a></span><span class="over">已过期</span></div></li>';
