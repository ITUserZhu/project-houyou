...
$(function(){
    ...
    var $navArr = $('#nav-arr'),
    	$navCol = $('.nav-col'),
    	$navAW = $navCol.find('.nav-a'),
    	$navA = $navAW.children('a'),
    	NavIndex = 0;
	$navAW.width($navA.width()*$navA.length);
	$navA.each(function(index, el) {
		if($(el).hasClass('active')) {
			var _index = $(el).index();
			if(_index > 11) {
				NavIndex = Math.ceil((_index+1)/12) - 1;
			}
			$navAW.css('marginLeft', -NavIndex*100 + '%');
		}
	});
	
    $navArr.click(function(e) {
    	var $target = $(e.target).closest('span'),
    		ArrLen = Math.ceil($navA.length / 12);
    	if($target.is('.left')) {
    		NavIndex--;
    		if(NavIndex < 0) {NavIndex++; return};
    	} else if ($target.is('.right')) {
    		NavIndex++;
    		if(NavIndex > ArrLen-1) {NavIndex--; return};
    	};
    	$navAW.animate({
			marginLeft: -NavIndex*100 + '%'
		},400);
    });
